...
void ListAlumnos(){
	...
	do{
		...
		//Realizando una consulta a la base de datos donde me muestra a todos los alumnos de un grado y seccion	
		system("cls");
		...
		db.text4="JOIN notas N ON A.IdAlumno=N.IdAlumno WHERE ASI.IdGrado="+db.IdGrado+" AND ASI.IdSeccion="+db.IdSeccion+"";
		db.sql=db.text1+db.text2+db.text3+db.text4;
		query = db.sql.c_str();
		db.resultado=mysql_query(conexion, query);
		
		//Imprimiendo los alumnos del grado y seccion definidos por el usuario
		if (db.resultado==0){
			data=mysql_store_result(conexion);
			int countColumns=mysql_num_fields(data);
			int countRows=mysql_num_rows(data);
			...
			//Muestro este mensaje si no se encuentra ningun alumno
			else{
				system("cls");
				system("color c");
				cout<<"\t\t\t\tNo se encontro alumnos asignados a este grado y seccion";
			}
		}
		...
		cout<<endl<<endl;
		cout<<"\t\t\t\tPresione '1' para volver a listar los alumnos de un grado y seccion"<<endl;
