...
void AgregarSeccion(){
	...
	do{
		...
		if (db.resultado==0){
			...
			cout<<endl<<endl;
			cout<<"\t\t\t\tIngrese un codigo para la nueva seccion: ";
			cin>>db.IdSeccion;
			cout<<"\t\t\t\tIngrese un nombre para la nueva seccion: ";
			cin>>db.Seccion;
				
			//Generando un nuevo bucle donde valido si los datos ingresados anteriormente se encuentran ya registrados
			do{		
				db.sql = "SELECT * FROM seccion WHERE IdSeccion="+db.IdSeccion+" OR Seccion='"+db.Seccion+"'";
				query = db.sql.c_str();
				db.resultado=mysql_query(conexion, query);
				data=mysql_store_result(conexion);
				...
				if(countRows>=1){
					...
					cout<<"\t\t\t\tEl codigo '"+db.IdSeccion+"' o nombre '"+db.Seccion+"' ya se encuentran ingresados\n\n\n";
					
					db.sql = "SELECT * FROM seccion";
					query = db.sql.c_str();
					db.resultado=mysql_query(conexion, query);
							
					//Imprimiendo de nuevo mi tabla seccion en tal caso los datos ingresados esten duplicados
					if (db.resultado==0){
						data=mysql_store_result(conexion);
						int countColumns=mysql_num_fields(data);
						int countRows=mysql_num_rows(data);
					...
					cout<<"\n\t\t\t\tIngrese un codigo diferente para la nueva seccion: ";
					cin>>db.IdSeccion;
					cout<<"\t\t\t\tIngrese un nombre diferente para la nueva seccion: ";
					cin>>db.Seccion;
					db.band=1;
				}
				...
				//Si los datos no se encuentran registrados, se registran automaticamente en la base de datos
					...
					db.sql="INSERT INTO seccion (IdSeccion, Seccion) VALUES ('"+db.IdSeccion+"', '"+db.Seccion+"')";
					query=db.sql.c_str();
					db.resultado=mysql_query(conexion, query);
					system("cls");
					system("color a");
		...
		cout<<endl<<endl;
		cout<<"\t\t\t\tPresione '1' para volver a agregar una seccion"<<endl;
